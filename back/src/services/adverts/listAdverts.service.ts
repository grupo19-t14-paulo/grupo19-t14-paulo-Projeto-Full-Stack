...
const listAdvertsService = async ({model, fuel, year, color, brand, maxPrice, minPrice, maxMileage, minMileage}): Promise<IAnnouncementResponse> => {
  const advertsRepository = await AppDataSource.getRepository(Adverts);
